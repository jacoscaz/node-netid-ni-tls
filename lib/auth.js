...
Authenticator.prototype.authenticate = function (clientCert) {
  ...
  return authenticator.fetch({netID: clientCert.netID, clientCert: clientCert.uri})
    .then(function (auth) {
    ...
    .catch(ERRORS.FailedAuthenticationError, function (err) {
      debug('Authentication error: %s: %s', err.name, err.message);
      return {
        success: false,
        netID: clientCert.netID,
        clientCert: clientCert.uri,
    ...
    .then(function (auth) {
      setImmediate(function () {
        authenticator.emit('authentication', auth);
      });
      return auth;
    });
